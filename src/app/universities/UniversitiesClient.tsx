...
export function UniversitiesClient({ initialUnis }: { initialUnis: UniversityRow[] }) {
  ...
  const allUnis = initialUnis.some(u => u.name_en === 'Al Qasimia University')
    ? initialUnis
    : [AQ_UNI, ...initialUnis]
    ...
    <>
      ...
      <div className="bg-surface">
        <div className="container-wide py-10">
          {filtered.length === 0 && filter !== 'ALL' && !search && !prog ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <h3 className="font-bold text-lg text-ink mb-2">
                {lang === 'ru' ? 'Скоро добавим университеты этой страны' : lang === 'uz' ? "Bu mamlakatning universitetlari tez orada qo'shiladi" : 'Universities for this country coming soon'}
              </h3>
              <p className="text-muted text-sm mb-6 max-w-sm">
                {lang === 'ru' ? 'Оставьте заявку — мы свяжемся, как только откроется приём' : lang === 'uz' ? "Ariza qoldiring — qabul ochilishi bilanoq bog'lanamiz" : "Leave an application — we'll contact you when admissions open"}
              </p>
              <Link href="/apply" className="btn btn-primary px-6 py-3">
                {lang === 'ru' ? 'Оставить заявку' : lang === 'uz' ? 'Ariza qoldirish' : 'Leave application'}
              </Link>
            </div>
          ) : filtered.length === 0 ? (
